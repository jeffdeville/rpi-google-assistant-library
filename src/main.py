...
# kodiurl = (
#     "http://" + str(configuration['Kodi']['ip']) + ":" + str(configuration['Kodi']['port']) + "/jsonrpc")
...
custom_wakeword = True  # I added this
# else:
# custom_wakeword = False

# models = configuration['Wakewords']['Custom_wakeword_models']
models = [os.path.join(ROOT_PATH, 'resources/i_award.umdl')] # I added this
    ...
    def main(self):
        ...
        with Assistant(credentials, device_model_id) as assistant:
            ...
            for event in events:
                ...
                #             hue_control(
                #                 str(usrcmd).lower(), str(i), str(hueconfig['lights_address'][str(i)]['ip']))
                #             break
                #     except Keyerror:
                ...
                #         assistant.stop_conversation()
                #         tasmota_control(
                #             str(usrcmd).lower(), name.lower(), tasmota_deviceip[num],
                #             tasmota_deviceportid[num])
                #         break
                # for i in range(1, numques + 1):
                #     try:
                #         if str(configuration['Conversation']['question'][i][0]).lower() in str(
                #                 usrcmd).lower():
                #             assistant.stop_conversation()
                ...
                #     if len(configuration['Domoticz']['Devices']['Name']) == len(
                #             configuration['Domoticz']['Devices']['Id']):
                #         for i in range(0, len(configuration['Domoticz']['Devices']['Name'])):
                #             if str(configuration['Domoticz']['Devices']['Name'][i]).lower() in str(
                #                     usrcmd).lower():
                #                 assistant.stop_conversation()
                #                 domoticz_control(
                #                     str(usrcmd).lower(), configuration['Domoticz']['Devices']['Id'][i],
                #                     configuration['Domoticz']['Devices']['Name'][i])
                #                 break
                #     else:
                ...
                # if (custom_action_keyword['Keywords']['Kickstarter_tracking'][0]
                ...
                # if (custom_action_keyword['Keywords']['YouTube_music_stream'][0]
                #     ).lower() in str(usrcmd).lower() and 'kodi' not in str(
                #         usrcmd).lower() and 'chromecast' not in str(usrcmd).lower():
                #     assistant.stop_conversation()
                #     vlcplayer.stop_vlc()
                ...
                #     assistant.stop_conversation()
                #     track()
                # if (custom_action_keyword['Keywords']['RSS'][0]).lower() in str(usrcmd).lower() or (
                #         custom_action_keyword['Keywords']['RSS'][1]).lower() in str(usrcmd).lower():
                #     assistant.stop_conversation()
                ...
                # if (custom_action_keyword['Keywords']['Pause_resume'][0]).lower() in str(usrcmd).lower() or (
                #         custom_action_keyword['Keywords']['Pause_resume'][1]).lower() in str(usrcmd).lower():
                #     assistant.stop_conversation()
                #     if vlcplayer.is_vlc_playing():
                ...
                #             vlcplayer.pause_vlc()
                #     if checkvlcpaused():
                #         if (custom_action_keyword['Keywords']['Pause_resume'][1]
                #             ).lower() in str(usrcmd).lower():
                #             vlcplayer.play_vlc()
                #     elif vlcplayer.is_vlc_playing() == False and checkvlcpaused() == False:
                #         say("Sorry nothing is playing right now")
                # if (custom_action_keyword['Keywords']['Track_change']['Next'][0]
                #     ).lower() in str(usrcmd).lower() or (
                #         custom_action_keyword['Keywords']['Track_change']['Next'][1]).lower() in str(
                #             usrcmd).lower() or (custom_action_keyword['Keywords']['Track_change']['Next'][2]
                #                                 ).lower() in str(usrcmd).lower():
                #     assistant.stop_conversation()
                ...
                #     elif vlcplayer.is_vlc_playing() == False and checkvlcpaused() == False:
                #         say("Sorry nothing is playing right now")
                # if (custom_action_keyword['Keywords']['Track_change']['Previous'][0]
                #     ).lower() in str(usrcmd).lower() or (
                #         custom_action_keyword['Keywords']['Track_change']['Previous'][1]
                #     ).lower() in str(usrcmd).lower() or (custom_action_keyword['Keywords']['Track_change']
                ...
                #         if (custom_action_keyword['Dict']['Set']).lower() in str(usrcmd).lower() or (
                #                 custom_action_keyword['Dict']['Change']).lower() in str(usrcmd).lower():
                #             if 'hundred'.lower() in str(usrcmd).lower(
                #             ) or custom_action_keyword['Dict']['Maximum'] in str(usrcmd).lower():
                #                 settingvollevel = 100
                #                 with open('{}/.mediavolume.json'.format(USER_PATH), 'w') as vol:
                #                     json.dump(settingvollevel, vol)
                #             elif 'zero'.lower() in str(usrcmd).lower(
                #             ) or custom_action_keyword['Dict']['Minimum'] in str(usrcmd).lower():
                #                 settingvollevel = 0
                #                 with open('{}/.mediavolume.json'.format(USER_PATH), 'w') as vol:
                #                     json.dump(settingvollevel, vol)
                #             else:
                ...
                #         elif (custom_action_keyword['Dict']['Increase']).lower() in str(usrcmd).lower() or (
                #                 custom_action_keyword['Dict']['Decrease']
                #         ).lower() in str(usrcmd).lower() or 'reduce'.lower() in str(usrcmd).lower():
                #             if os.path.isfile("{}/.mediavolume.json".format(USER_PATH)):
                #                 with open('{}/.mediavolume.json'.format(USER_PATH), 'r') as vol:
                ...
                #     else:
                #         say("Sorry nothing is playing right now")
                # if (custom_action_keyword['Keywords']['Music_index_refresh'][0]).lower() in str(
                #         usrcmd).lower() and (custom_action_keyword['Keywords']['Music_index_refresh'][1]
                #                              ).lower() in str(usrcmd).lower():
                #     assistant.stop_conversation()
                #     refreshlists()
                # if (custom_action_keyword['Keywords']['Google_music_streaming'][0]
                #     ).lower() in str(usrcmd).lower():
                #     assistant.stop_conversation()
                #     vlcplayer.stop_vlc()
                #     gmusicselect(str(usrcmd).lower())
                # if (custom_action_keyword['Keywords']['Spotify_music_streaming'][0]
                #     ).lower() in str(usrcmd).lower():
                #     assistant.stop_conversation()
                #     vlcplayer.stop_vlc()
                #     spotify_playlist_select(str(usrcmd).lower())
                # if (custom_action_keyword['Keywords']['Gaana_music_streaming'][0]
                #     ).lower() in str(usrcmd).lower():
                #     assistant.stop_conversation()
                #     vlcplayer.stop_vlc()
                #     gaana_playlist_select(str(usrcmd).lower())
                # if (custom_action_keyword['Keywords']['Deezer_music_streaming'][0]
                #     ).lower() in str(usrcmd).lower():
                #     assistant.stop_conversation()
                #     vlcplayer.stop_vlc()
